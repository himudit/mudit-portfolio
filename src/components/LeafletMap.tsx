...
const TimelineItem = ({
  ...
  <div
    onClick={onClick}
    className={cn(
      'cursor-pointer p-4 border-l-4 transition-colors flex flex-col gap-2',
      isActive
        ? 'bg-yellow-100 dark:bg-gray-700 border-black'
        : 'hover:bg-gray-100 dark:hover:bg-gray-600 border-transparent',
    )}
  >
    ...
    <p className="text-sm flex items-center gap-1">
      <MapPin size={14} /> {entry.locationName}
    </p>
  </div>
);
...
const TimelineContainer = ({
  ...
  <div className="overflow-y-auto p-4 md:w-[420px] border-r-4 border-black">
    {entries.map((entry, index) => (
      <TimelineItem
        key={entry.id}
        entry={entry}
...
const MapComponent: React.FC = () => {
  ...
  const selectedLocation =
    selectedIndex >= 0 ? timelineData[selectedIndex].location : null;
  ...
  return (
    <div className="relative h-[700px] border-4 border-black overflow-hidden">
      <div className="flex h-full">
        ...
        <MapContainer
          center={[17.445, 78.3915]}
          zoom={5}
          scrollWheelZoom
          style={{ height: '100%', width: '100%' }}
          whenReady={() => {
            // map is ready, but not passed here
          }}
        >
      ...
      {isMobile && (
        <button
          ...
          className="absolute top-4 left-4 z-30 p-3 border-4 border-black bg-white"
        >
          <Menu size={24} />
        </button>
      )}
